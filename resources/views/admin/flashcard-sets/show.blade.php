<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="space-x-4">
                 <a href="{{ route('admin.flashcard-sets.index') }}" class="text-sm text-gray-600 hover:text-gray-900 underline">
                    {{ __('Back to All Sets') }}
                </a>
                <a href="{{ route('admin.flashcard-sets.edit', $flashcardSet) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
                    {{ __('Edit Set') }}
                </a>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $flashcardSet->title }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Created By</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $flashcardSet->creator->name }} ({{ $flashcardSet->creator->email }})</dd>
                        </div>
                        <div>
                ...
                <div class="p-6 text-gray-900">
                     <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="space-x-2">
                            ...
                            <a href="{{ route('admin.flashcard-sets.flashcards.create', $flashcardSet) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ __('Add New Card') }}
                            </a>
                            <a href="{{ route('admin.flashcard-sets.flashcards.index', $flashcardSet) }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
                                {{ __('Manage Cards') }}
                            </a>
                        </div>
                    </div>
                                ...
                                @forelse ($flashcardSet->flashcards as $card)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.flashcard-sets.flashcards.edit', [$flashcardSet, $card]) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                                            <form action="{{ route('admin.flashcard-sets.flashcards.destroy', [$flashcardSet, $card]) }}" method="POST" class="inline-block ml-4" onsubmit="return confirm('Are you sure you want to delete this flashcard?');">
                                                @csrf
                                                @method('DELETE')
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg" x-data="{ isUploading: false }">
                <div class="p-6">
                    ...
                    <form 
                        action="{{ route('admin.flashcard-sets.flashcards.bulk-import', $flashcardSet) }}" 
                        method="POST" 
