<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.flashcard-sets.show', $flashcardSet) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                {{ __('Back to Set') }}
            </a>
        </div>
    </x-slot>
                                ...
                                @forelse ($flashcards as $flashcard)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('admin.flashcard-sets.flashcards.edit', [$flashcardSet, $flashcard]) }}" class="text-yellow-600 hover:text-yellow-900 mr-4">Edit</a>
                                            <form action="{{ route('admin.flashcard-sets.flashcards.destroy', [$flashcardSet, $flashcard]) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this flashcard?');">
                                                @csrf
                                                @method('DELETE')
